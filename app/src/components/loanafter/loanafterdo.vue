        ...
        <div class="panel-body">
          <div class="row searchDiv">
            <div class="col-lg-3 col-md-3 col-xs-12">
              <span>客户名称：</span><input v-model="search.customerName" type="text" name="productName"/>
            </div>
            <div class="col-lg-3 col-md-3 col-xs-12">
              <span>证件号码：</span><input v-model="search.customerCardNum" type="text" name="productState"/>
            </div>
          ...
          <div class="tableDiv">
            <table class="table table-striped table-bordered table-hover order-column" id="dtUsers">
              ...
              <tr v-for="info in infos">
                ...
                <td>${info.customerCardNum}</td>
                <td>${info.productName}</td>

                <td>${info.applyAmount}</td>
                <td>${info.tPostLoanMonitorType}</td>
                <td><a class="btn btn-info btn-xs" v-on:click="editInfo(info.id)">贷后监控</a></td>
              </tr>
              </tbody>
            </table>
          </div>
    ...
    export default{
        ...
        methods:{
           ...
           editInfo:function(id){
              ...
              QK.noteNowUrl()
              //跳转地址
              this.$router.go({path: '/system/loanafter/loanmonitor/' + id})
           }
        }
    }
